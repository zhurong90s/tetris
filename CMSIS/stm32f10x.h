/**
  ...
  *
  *    翻译：
  *			此头文件遵循CMSIS协议，是Cortex-M3设备外设层头文件。
  *			它包含了所有外设寄存器和寄存器位的定义，及内存映射。
  *			涵盖了低、中、高密度型，超值型，互联型等设备。
  *
  *			用户的C语言应用程序只需要包含此头文件。通常应用程序被命名为
  *			mian.c。此文件包含：
  *			——包含应用程序中用到的外备驱动程序声明
  *			——是否使用外设驱动程序，取决于是否定义了"#define USE_STDPERIPH_DRIVER"
  *			  这个宏。具体使用那些硬件在stm32f10x_conf.h中定义。
  *			——可以改变少数特殊的参数，例如HSE的频率
  *			——外设寄存器及位的宏定义定义
  *			——定义所有外设的地址与寄存器结构体件的映射。
  *         ——可以通过定义的宏实现对硬件的操作
  *
  ******************************************************************************
  * @attention
...
/*  Tip: To avoid modifying this file each time you need to switch between these
 ...
 - Connectivity line devices are STM32F105xx and STM32F107xx microcontrollers.
   ...
   翻译：
   建议：尽量避免对此文件的修改。你可以在编译器中定义你的设备。
 - STM32F101xx,STM32F102xx 和 STM32F103xx的低密度型Flash在16k~32k。
 - STM32F100xx的低密度超值型Flash在16k~32k。
 - STM32F101xx,STM32F102xx 和 STM32F103xx的中密度型Flash在64k~128k。
 - STM32F100xx的中密度超值型Flash在64k~128k。 
 - STM32F101xx和STM32F103xx的高密度型Flash在256k~512k。
 - STM32F100xx的高密度超值型Flash在256k~512k。
 - STM32F101xx和STM32F103xx的超高密度超值型Flash在512k~1024k。
 - STM32F105xx和STM32F107xx为互联型。
 ...
 * @brief Comment the line below if you will not use the peripherals drivers.
   ...
   翻译：
   注释掉下面的宏你将无法使用外设驱动，和外设有关的一些结构体。
   在这种情况下外设驱动程序和对应的头文件将不能使用，应用程序只能
   直接对寄存器进行操作。
*/
 ...
 * @brief In the following line adjust the value of External High Speed oscillator (HSE)
   ...
   翻译：下面几行可以调整应用程序中使用的外部时钟HSE频率
   建议：避免对下面代码的调整，如果需要调整，可以再编译器中进行设置
